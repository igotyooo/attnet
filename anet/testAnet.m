...
%% MAKE TEST DB.
clearvars -except adb anet db det path res setting;
dbdir = '/home/dgyoo/workspace/datain/CITYSCAPES/';
dbname = 'CITYSCAPES';
iid2impath = dir( fullfile( dbdir, '*.png' ) );
iid2impath = fullfile( dbdir, { iid2impath.name }' );
iid2size = zeros( 2, numel( iid2impath ) );
parfor iid = 1 : numel( iid2impath ),
    [ r, c, ~ ] = size( imread( iid2impath{ iid } ) );
    iid2size( :, iid ) = [ r; c; ];
end;
iid2setid = 2 * ones( size( iid2impath ) );
det.db.iid2impath = iid2impath;
det.db.iid2size = iid2size;
det.db.iid2setid = iid2setid;
det.db.name = dbname;
det.db.dstDir = '/home/dgyoo/workspace/dataout/attnet/CITYSCAPES';
