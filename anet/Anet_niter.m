classdef Anet_niter < handle
    ...
    methods( Access = public )
        ...
        function [ rid2tlbr, rid2score, rid2cid, nstop, niter ] = iid2det0( this, iid )
            ...
            nstop = size( rid2tlbr, 2 );
            niter = sum( min( rid2tlbr( 5, : ), 50 ) );
            rid2tlbr = rid2tlbr( 1 : 4, : );
        ...
        function res = getSubDbDet0( this, numDiv, divId )
            ...
            iid2nstop = zeros( numIm, 1 );
            iid2niter = zeros( numIm, 1 );
            rid2iid = cell( numIm, 1 );
            cummt = 0;
            for iidx = 1 : numIm; itime = tic;
                iid = idx2iid( iidx );
                [ rid2tlbr{ iidx }, rid2score{ iidx }, rid2cid{ iidx }, iid2nstop( iidx ), iid2niter( iidx ) ] = this.iid2det0( iid );
            ...
            res.did2cid = rid2cid;
            res.meanNumIter = sum( iid2niter ) / sum( iid2nstop );
            res.did2iid = rid2iid;
        end;
    ...
    methods( Access = private )
        ...
        function [ rid2tlbr, rid2out, rid2cid, fid2boxes ] = iid2det...
            ...
            rid2tlbr( 1 : 4, : ) = bsxfun( @minus, rid2tlbr( 1 : 4, : ), 1 - [ imTl; imTl; ] );
            if nargout == 4,
        ...
        function [ did2tlbr, did2out, did2cid, fid2boxes ] = dynamicFitting...
            ...
            for feed = 1 : numMaxFeed,
                ...
                for cid = 1 : numCls,
                    ...
                    did2tlbr( :, dids ) = cat( 1, rid2tlbr( :, rid2det ), feed * ones( 1, numDet ) );
                    did2cid( dids ) = cid;
                    did2niter( dids ) = feed;
        ...
        function name = getDet1Name( this )
            name = sprintf( 'DET1NI_%s_OF_%s_OF_%s', ...
